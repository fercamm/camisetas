...
//sanitizo variables POST
    ...
    $email = $_POST["email"];
    $telefono = $_POST["telefono_pedido"];
    ...
    $cantidadCamisetasJugadores = 0;
    $cantidadCamisetasArqueros = 0;

    $camisetas = json_decode(json_decode('"'.$_POST["camisetas"].'"'));
    $shorts = json_decode(json_decode('"'.$_POST["shorts"].'"'));
    $conjuntos = json_decode(json_decode('"'.$_POST["conjuntos"].'"'));

    //Esto es solamente para detectar errores:
    if($genero != 'chombas'){
      if((count($camisetas) + count($shorts) + count($conjuntos)) == 0){
    ...
    //                        , "' . $_POST["hPUid"] . '" , ' . $_POST["hPConj"] . ' , ' . $_POST["hPArquero"] . ' , ' . $_POST["hTotal"] . ', UUID() )';
      ...
      if($genero != 'chombas'){
        $query = 'UPDATE pedidos SET cliente_id = '. $clienteId . ', fecha = "' . $fecha . '", diseno = ' . $diseno . ', 
          ...
          precio_uuid = , "' . $precios->uuid . '" , precio_conjunto = ' .($precioCamiseta + $precioShort + $precioMedia * intval($_POST["option_medias_on"])). ' , precio_arquero = ' . ($precioCamiseta + $precioShort + $precioMedia * intval($_POST["option_medias_on"])) . ' , precio_total = ' . $total . ', 
    ...
    for($i = 0; $i < count($conjuntos) ;$i++ ){
        ...
        if(!empty($arquerosJson) && $conjuntos[$i]->arquero){
            $arquerosJson .= ',';
        }
        if(!empty($jugadoresJson) && !$conjuntos[$i]->arquero){
            $jugadoresJson .= ',';
        }
    
        $jugador = guardarConjunto($pedido, $conjunto, 'conjunto');
        $conjuntos[$i]->id = $jugador['id'];
    
        if($conjuntos[$i]->arquero){
          $cantidadCamisetasArqueros++;
            $arquerosJson .= '
                {
                "talle": "'.$conjunto->talle_camiseta.'",
                "nombre": "'.$conjunto->nombre.'",
                "numero": "'.$conjunto->numero.'",
                "talle_short": "'.$conjunto->talle_short.'"
                }';
        }else{
          $cantidadCamisetasJugadores++;
            $jugadoresJson .= '
                {
                "talle": "'.$conjunto->talle_camiseta.'",
                "nombre": "'.$conjunto->nombre.'",
    ...
    for($i = 0; $i < count($camisetas) ;$i++ ){
        ...
        if(!empty($arquerosJson) && $camisetas[$i]->arquero){
            $arquerosJson .= ',';
        }
        if(!empty($jugadoresJson) && !$camisetas[$i]->arquero){
            $jugadoresJson .= ',';
        }
    
        $jugador = guardarConjunto($pedido, $camiseta, 'camiseta');
        $camisetas[$i]->id = $jugador['id'];
    
        if($camisetas[$i]->arquero){
          $cantidadCamisetasArqueros++;
            $arquerosJson .= '
                {
                "talle": "'.$camiseta->talle_camiseta.'",
    ...
    for($i = 0; $i < count($shorts) ;$i++ ){
        $short = $shorts[$i];
        if(!empty($arquerosJson) && $shorts[$i]->arquero){
            $arquerosJson .= ',';
        }
        if(!empty($jugadoresJson) && !$shorts[$i]->arquero){
            $jugadoresJson .= ',';
        }
    
        $jugador = guardarConjunto($pedido, $short, 'short');
        $shorts[$i]->id = $jugador['id'];
...
//------
        ...
        <tbody>
          <tr>
            <td style="padding: 30px 0;color: #656565;font-family: sans-serif;">
              <img src="https://'.$genero.'.'.$url_server.'/'.$genero.'/img/logo_yakka.png" alt="Yakka">
            </td>
          </tr>
        </tbody>
      </table>
        ...
        <tbody>
          <tr>
            <td class="contenido" style="font-size: 18px;padding: 10px 30px 0px 30px;line-height:21px;text-align: center;font-family: "Source Sans Pro", sans-serif;color: #161616;" align="center">
             A la brevedad un asesor se va a comunicar y enviarte el presupuesto. Podes ver tu pedido haciendo <a href="https://pedido.'.$url_server.'/pedido-'.$pedidoUuid.'" target="_blank" title="Ver pedido" style="color:#3096fe; font-family: "Source Sans Pro", sans-serif; font-style: italic; font-weight: 700;text-decoration: none;" >click aquí</a></td>
             </tr>
             <tr>
        ...
        <tbody>
          <tr>
            <td style="padding: 0px 30px 40px 30px;" align="center">
              <a href="https://pedido.'.$url_server.'/pedido-'.$pedidoUuid.'" target="_blank"><img src="https://'.$genero.'.'.$url_server.'/'.$genero.'/img/boton_mailing.jpg" alt="Ver Pedido" /></a>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <tr>
      ...
      <table align="center" style="background: #fdfdfd;width: 90%;">
        <tbody>
          <tr>
            <td class="contenido" style="font-size: 18px;padding: 10px 30px 0px 30px;line-height:21px;text-align: center;font-family: "Source Sans Pro", sans-serif;color: #161616;" align="center">
              Tenés un nuevo pedido de '. ($genero == 'chombas' ? 'chombas' : 'camisetas') .' mirá los detalles haciendo <a href="https://pedido.'.$url_server.'/pedido-'.$pedidoUuid.'" target="_blank" title="Ver pedido" style="color:#3096fe; font-family: "Source Sans Pro", sans-serif; font-style: italic; font-weight: 700;text-decoration: none;" >click aquí</a>
            </td>
          </tr>
       </tbody>
      </table>
  ...
  mail('fercamm@gmail.com','Tu pedido de camisetas - Yakka',$message,$headers); // al user
  
  mail('info@yakka.com.ar','¡Nuevo pedido ingresado!',$messageToYakka,$headers); // a yakka
  mail('fercamm@gmail.com','¡Nuevo pedido ingresado!',$messageToYakka,$headers); // a yakka
